...
var token;
genToken = function(accessKey, secretKey, putPolicy, showStep) {
    var setStep = function(id, val, showStep) {
        if (showStep) {
    ...
    setStep("disp_step2", put_policy, showStep);

    //SETP 3
    var encoded = base64encode(utf16to8(put_policy));
    console.log("encoded = ", encoded);
    setStep("disp_step3", encoded, showStep);

    //SETP 4
    var hash = CryptoJS.HmacSHA1(encoded, secretKey);
    var encoded_signed = hash.toString(CryptoJS.enc.Base64);
    setStep("disp_step4", encoded_signed, showStep);

    //SETP 5
    var upload_token = accessKey + ":" + safe64(encoded_signed) + ":" + encoded;
    setStep("disp_step5", upload_token, showStep);
...
var tokenPanel_policy = new Ext.form.Panel({
    ...
    items: [{
        ...
        handler: function(btn) {
            ...
            token = genToken(accessKey, secretKey, policy, true);
            Ext.getCmp("token_res").expand()
            Ext.getCmp("btn_useToken").show();
            console.log("token=", token);
        }
    }, {
        xtype: 'displayfield',
        value: '文档地址：<a target="_blank" href="http://docs.qiniu.com/api/v6/put.html#uploadToken">http://docs.qiniu.com/api/v6/put.html#uploadToken</a><b</b>'
    }]
});
...
var tokenPanel = Ext.create('Ext.panel.Panel', {
    ...
    items: [tokenPanel_policy, {
        ...
        items: [{
            ...
            text: "使用此token上传文件",
            handler: function() {
                var v = Ext.getCmp("txt_key").getValue();
                if (v) {
                    Ext.getCmp("txt_upload_key").setValue(v);
                }
                Ext.getCmp("txt_upload_token").setValue(token);
                Ext.getCmp("tabContainer").setActiveTab(1);
            }
        }]
    }]
});
