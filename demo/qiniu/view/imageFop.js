var FomrItem;
var formEncoded = function(FormPanel) {
    var res = "";
    FormPanel.items.each(function(item, index, length) {
        if (!item.noCal) {
            var name = item.trueName ? item.trueName : item.name;
            if (item.xtype == "radiogroup") {
                var v = item.getValue()[item.items.items[0].name];
                if (v) {
                    res += "/" + name + "/";
                    if (item.B64) {
                        res += base64encode(utf16to8(v));
                    } else {
                        res += v;
                    }
                }
...
var gravity = function(name,label) {
    return {
        xtype: 'radiogroup',
        trueName: "gravity",
        name: name,
        ...
        items: [{
            inputValue: 'NorthWest',
            boxLabel: 'NorthWest'
        }, {
            inputValue: 'North',
            boxLabel: 'North'
        }, {
            inputValue: 'NorthEast',
            boxLabel: 'NorthEast'
        }, {
            inputValue: 'West',
            boxLabel: 'West'
            ...
            inputValue: 'SouthWest',
            boxLabel: 'SouthWest'
        }, {
            inputValue: 'South',
            boxLabel: 'South'
        }, {
            inputValue: 'SouthEast',
            boxLabel: 'SouthEast'
        }]
    }
};

Ext.define('EP.view.qiniu.imageFop', {
    extend: 'Ext.form.Panel',
    alias: 'widget.imageFop',
    layout: 'border',
    glyph: 42,
    autoScroll:true,
    initComponent: function() {
        var loadImg = function(v) {
            ...
            panel_ImgInfo.update('<iframe width="100%" height="80%" id="frame_imgInfo" src="' + v + '?imageInfo" />');
            panel_ExifInfo.update('<iframe id="iframeexif"  width="100%" height="80%" id="frame_exifInfo2" src="' + v + '?exif" />')
        ...
        var form = new Ext.form.Panel({
            ...
            items: [{
                ...
                vtype: 'url',
                value: 'http://cyj.qiniudn.com/22734/1359639667984p17i8ddoi31ara1ccp1njsq319s62.jpg',
                id: "txt_imgSrcHref"
            }, {
                xtype: 'button',
                text: "加载图片",
                ...
                items: [{
                    xtype: 'image',
                    id: 'img_disp',
                    src: 'http://cyj.qiniudn.com/22734/1359639667984p17i8ddoi31ara1ccp1njsq319s62.jpg'
                }]
            }],
            bbar: [{
                fieldLabel: "预览地址",
